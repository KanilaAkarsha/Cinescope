  ...
  return (
    ...
    <Card className="border-primary/20 hover:border-primary/50 overflow-hidden py-0 gap-0 transition-colors">
        <div className="aspect-2/3 w-full overflow-hidden">
            <Image width={300} height={450} src={posterUrl || "/placeholder.svg"} alt={movie.title} 
            className="h-full w-full object-cover transition-transform duration-300 hover:scale-105" priority onError={() => setPosterUrl("/placeholder.svg")}/>
        </div>
        <CardContent className="p-4">
            ...
            <div className="flex flex-wrap gap-1 mt-2">
                {movie?.genres?.slice(0,2).map((genre,index) =>(
                    <Badge variant="outline" key={`${genre}-${index}`} className=" border-primary/30 bg-primary/5 text-xs">{genre}</Badge>
                )) }
